...
```bash
docker compose exec abtool ab -n 500 -c 20 http://host.docker.internal:8888/
```
...
```bash
docker compose exec abtool ab -n 500 -c 20 http://host.docker.internal:8888/ | tee resultado-lectura.txt | grep -E "Requests per second|Failed requests|Time per request"
```
